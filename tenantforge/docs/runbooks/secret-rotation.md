...
_Last validated: 2026-06-18 — **NEON_API_KEY rotation** drilled against a non-prod org (game-day suite 10/10 on the **rotated** key) and **DATABASE_URL (registry-credential) rotation** drilled non-destructively against a non-prod registry: add-new-before-revoke-old proven via a throwaway least-privilege role — both old and new credentials read the registry concurrently (dual-valid window), then revoke + `DROP ROLE` proved a rotated credential is rejected, with the primary credential untouched. Per-tenant connection secrets were also set/resolved/shredded end-to-end. Revoking the old API key / rotating the live DSN credential are the operator's Neon console/API steps after verification. See [drill-report](./drill-report.md). Owner: TenantForge maintainers._
