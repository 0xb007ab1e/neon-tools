...
> residency, an audit excerpt). STRIDE pass below. **Out of scope here (Phase 3):** persistence
> (`EvidenceStore`), the **retrieval surface + access control** (BOLA on _fetch_), the public-key
> endpoint, the generate webhook, and the dashboard panel — those land later; this section covers the
> assembly + sign + verify boundary.
...
_Last reviewed: 2026-06-25 (ADR-0011 Phase 2 — evidence bundle assembly + sign + verify, fleet + per-tenant; B10 added). Owner: TenantForge maintainers. Review on any trust-boundary change._
